    ...
    return (
        <div className="mt-6 space-y-4">
            <h4 className="text-lg font-semibold text-gray-900 flex items-center">
                <Award className="h-5 w-5 mr-2 text-primary-600"/>{t("jury_scores_evaluation")}
            ...
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="flex-1">
                    <p className="font-medium text-gray-900">
                        {scoredCount} / {totalJuries}{t("scores_submitted")}
                    </p>
                ...
                {finalScore !== null && finalScore !== undefined &&
                    <div className="text-right">
                        <p className="text-sm text-gray-500">{t("final_score")}</p>
                                ...
                                <div className="flex items-center">
                                    ...
                                    <div className="ml-3">
                                        <p className="font-medium text-gray-900 flex items-center gap-2">
                                            ...
                                            {isCurrentUser &&
                                                <span
                                                    className="text-xs bg-primary-100 text-primary-700 px-2 py-0.5 rounded-full">{t("you")}

                      </span>
                                            }
                                            {isInstructor &&
                                                <span
                                                    className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">{t("instructor")}
            ...
            {canSubmitScore &&
                <Card className="p-4 border-2 border-primary-200 bg-primary-50">
                    <h5 className="font-medium text-gray-900 mb-3 flex items-center">
                        <Send className="h-4 w-4 mr-2 text-primary-600"/>{t("submit_your_score")}
                    ...
                    <div className="flex items-end gap-4">
                        <div className="flex-1">
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("score_020")}
                    ...
                    <p className="mt-2 text-xs text-yellow-700">{t("_once_submitted_your_score_cannot_be_changed")}
            ...
            {isCurrentUserJury && hasCurrentUserScored && meetingState === MeetingState.SCHEDULED &&
                <div className="flex items-center p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div>
                        <p className="font-medium text-green-800">{t("your_score_submitted")}
                            {currentUserScore?.toFixed(1)} / 20
                        </p>
                        {pendingCount > 0 &&
                            <p className="text-sm text-green-600">{t("waiting_for")}
                                {pendingCount}{t("more_jury_member")}{pendingCount > 1 ? 's' : ''}
                            </p>
                        }
                    </div>
                </div>
            ...
            {meetingState === MeetingState.COMPLETED && finalScore !== null &&
                <Card className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 text-center">
                    ...
                    <h5 className="text-lg font-semibold text-gray-900">{t("defense_completed")}</h5>
                    <p className="text-3xl font-bold text-green-600 mt-2">
                        {Number(finalScore).toFixed(2)} / 20
                    </p>
                    <p className="text-sm text-gray-500 mt-1">{t("final_average_score")}</p>
                </Card>
            }
        </div>);
