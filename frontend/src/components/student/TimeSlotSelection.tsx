...
export const TimeSlotSelection: React.FC<TimeSlotSelectionProps> = ({meetingId}) => {
    ...
    if (Object.keys(groupedSlots).length === 0) {
        return (
            <Card className="bg-gray-50">
                <p className="text-gray-500 text-center py-8">{t("no_common_time_slots_available_yet_waiting_for_jur")}
    ...
    return (
        <div className="space-y-6">
            <h4 className="text-lg font-semibold text-gray-900">{t("select_your_time_slot")}</h4>
            ...
            <div className="flex gap-2 flex-wrap">
                <Button
                    ...
                    size="sm"
                    onClick={() => setSelectedDate('')}>{t("all_dates")}
            ...
            <div className="space-y-4">
                {Object.entries(groupedSlots).filter(([date]) => !selectedDate || date === selectedDate).map(([date, slots]) =>
                            ...
                            {(slots as TimeSlot[]).map((slot: TimeSlot) =>
                                <div
                                    ...
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="text-sm font-medium text-gray-900">{t("time_slot")}</div>
                                    ...
                                    {selectedSlot === slot.id &&
                                        <Button
                                            ...
                                            isLoading={selectSlotMutation.isPending}>{t("confirm_selection")}
